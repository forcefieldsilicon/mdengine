...
public struct HostedJobSpec: Codable {
    ...
    /// Runner flavour on the endpoint's allowlist ("lammps" default, "openmm"); nil = endpoint default.
    public var runner: String?

    public init(input: String, label: String? = nil, gpu: String = "any",
                wallLimitS: Int = 14400, estimateS: Int = 3600, launch: String = "default", runner: String? = nil) {
        self.input = input; self.label = label; self.gpu = gpu
        self.wall_limit_s = wallLimitS; self.estimate_s = estimateS; self.launch = launch; self.runner = runner
    }
}
