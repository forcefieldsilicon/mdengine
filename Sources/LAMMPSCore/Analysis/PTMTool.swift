...
public struct PTMTool: AnalysisTool {
    ...
    public static func analyze(frame: Frame, context: AnalysisContext,
        ...
        let match = try classify(frame: frame, context: context, templates: templates,
                                 rmsdThreshold: params.rmsdThreshold)
        let (classes, rmsd, shear, orientation, list) =
            (match.classes, match.rmsd, match.shear, match.orientation, match.neighbors)
        ...
        let segmentation = try GrainSegmentation.compute(count: n, stride: stride, classes: classes,
                                                         orientation: orientation, neighbors: list,
                                                         gbRadians: gbRadians,
                                                         isCancelled: context.isCancelled)
        let isGrainBoundary = segmentation.isBoundary
        let grains = segmentation.grainCount(minimumSize: minimumGrainSize)
        ...
        let meanGBAngle = segmentation.meanBoundaryAngle * 180 / Double.pi
    ...
    /// The per-atom PTM answer — classes, RMSDs, shears and lattice orientations —
    /// plus the neighbour list they were measured on. `GrainBoundaryTool` needs
    /// the orientations and nothing else PTM reports, so the match loop lives
    /// here rather than inside `analyze` (GJOB-235).
    struct Classification {
        let classes: [PTMClass]
        let rmsd: [Double]
        let shear: [Double]
    ...
    static func classify(frame: Frame, context: AnalysisContext,
                         templates: [PTMTemplate], rmsdThreshold: Double) throws -> Classification {
        let n = frame.count
        ...
        if context.isCancelled() { throw AnalysisError.cancelled }
        return Classification(classes: classes, rmsd: rmsd, shear: shear,
                              orientation: orientation, neighbors: list)
    }
