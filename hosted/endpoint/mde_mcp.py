...
Design (stdlib only, no sessions):
  ...
  * `initialize`, `ping`, `tools/list` need no key so directories and clients can inspect the server.
    `tools/call` without a valid credential returns HTTP 401 + WWW-Authenticate pointing at the protected
    resource metadata, which makes MCP clients start the OAuth flow implemented in mde_oauth.py.
...
SPEC_PROPS = {
    ...
    "gpu": {"type": "string", "description": "GPU class; call account for the currently offered classes and hourly rates. Default any = cheapest available"},
    ...
    "runner": {"type": "string", "description": "Runner flavour: lammps (default) or openmm (beta)"},
...
TOOLS = [
    ...
    {"name": "submit_job", "title": "Submit a deck (inline files) and start it",
     "description": "One call: create a hosted GPU job, upload the deck given INLINE as {relative_path: text}, and queue it. "
                    ...
                    "Billing starts at the first heartbeat (state running) and stops at done/failed/cancelled. The deck is executed as a program on an isolated GPU pod.",
    ...
    {"name": "list_jobs", "title": "List jobs",
     ...
     "inputSchema": {"type": "object", "properties": {"limit": {"type": "integer", "description": "1..200, default 20"},
                                                      "full": {"type": "boolean", "description": "Return the complete status object per job (default false)"}}},
     "annotations": _ann(True, idempotent=True)},
    {"name": "delete_results", "title": "Delete a job's files now",
...
for _t in TOOLS:                       # Claude's directory reads the title from annotations.title (ToolAnnotations); keep both
    _t["annotations"]["title"] = _t["title"]
...
class MCP:
    ...
    def handle(self, raw, key=None, bearer=None):
        ...
        if method == "initialize":
            ...
            return 200, self.ok(mid, {"protocolVersion": want if want in PROTOCOL_VERSIONS else DEFAULT_PROTOCOL,
                                      ...
                                      "instructions": "Hosted GPU runs for MDEngine. Every tool except this handshake needs a credential: sign in "
                                                      "through OAuth (paste your API key once) or send `Authorization: Bearer mde_...` (key from a credit pack at " + SIGNUP_URL + "). "
        ...
        if method == "tools/call":
            ...
            if key is None:                                   # HTTP 401 + WWW-Authenticate (added by the handler) starts the OAuth flow
                E.log("mcp.call", tool=name, key_id=None, ok=False, error="no_credential" if not bearer else "bad_credential")
                return 401, {"error": "unauthorized", "error_description": ("Unknown or expired credential." if bearer else "No credential.") +
                             " Sign in through OAuth (your MCP client offers it), or send Authorization: Bearer mde_... with an API key from " + SIGNUP_URL + "."}
            try:
                out = self.call(name, args, key); E.log("mcp.call", tool=name, key_id=key["key_id"], ok=True)   # never args: decks are user data
                return 200, self.ok(mid, self.tool_ok(out))
            except ToolError as e:
                E.log("mcp.call", tool=name, key_id=key["key_id"], ok=False, error=str(e)[:120])
                return 200, self.ok(mid, self.tool_error(str(e)))
    ...
    @staticmethod
    def tool_ok(obj): return {"content": [{"type": "text", "text": json.dumps(obj, separators=(",", ":"))}], "structuredContent": obj, "isError": False}
    @staticmethod
    ...
    def call(self, name, args, key):
        ...
        if name == "list_jobs":
            try: limit = max(1, min(int(args.get("limit") or 20), 200))
            ...
            if args.get("full") is True: return {"jobs": [E.status(j) for j in rows]}
            keep = ("id", "state", "label", "created", "finished", "gpu", "cost_usd", "error")
            return {"jobs": [{k: st[k] for k in keep} for st in (E.status(j) for j in rows)], "note": "full=true for complete status objects"}
        ...
        if name == "delete_results": return self.unwrap(*app.delete_job_files(self.job_for(args, key)))
