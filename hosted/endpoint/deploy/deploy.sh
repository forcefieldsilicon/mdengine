...
# rsyncs mde_endpoint.py + mde_admin.py + mde_launcher.py + mde_mcp.py + deploy/ to the host, runs bootstrap.sh (idempotent),
...
python3 -m py_compile "$SRC/mde_endpoint.py" "$SRC/mde_admin.py" "$SRC/mde_launcher.py" "$SRC/mde_mcp.py" "$SRC/mde_oauth.py"
...
rsync -az --delete --exclude '__pycache__' --exclude '*.pyc' \
  "$SRC/mde_endpoint.py" "$SRC/mde_admin.py" "$SRC/mde_launcher.py" "$SRC/mde_mcp.py" "$SRC/mde_oauth.py" "$SRC/deploy" "$TARGET:$STAGE/"
