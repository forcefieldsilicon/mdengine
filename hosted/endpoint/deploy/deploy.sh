...
python3 -m py_compile "$SRC/mde_endpoint.py" "$SRC/mde_admin.py" "$SRC/mde_launcher.py" "$SRC/mde_mcp.py"
...
rsync -az --delete --exclude '__pycache__' --exclude '*.pyc' \
  "$SRC/mde_endpoint.py" "$SRC/mde_admin.py" "$SRC/mde_launcher.py" "$SRC/mde_mcp.py" "$SRC/deploy" "$TARGET:$STAGE/"
