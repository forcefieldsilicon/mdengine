...
def cmd_key_revoke(a):
    """Delete the key row so the key can never authenticate again. Ledger/job rows stay when the key ever
    ran a job (statements need them); a never-used key is removed together with its credit rows so no
    phantom balance survives. Refuses while a job is still live unless --force (the pod keeps running)."""
    db, _ = open_db(a)
    k = db.key_by_id(a.key)
    if not k: sys.exit(f"no key {a.key}")
    jobs = db.q("select * from jobs where key_id=?", a.key)
    live = [j["id"] for j in jobs if j["state"] not in E.TERMINAL]
    if live and not a.force: sys.exit(f"key {a.key} has live jobs {live}: cancel them first, or pass --force (pods keep running)")
    bal = db.balance(a.key)
    with db.lock:
        db.x("update oauth_tokens set revoked=1 where key_id=?", a.key); db.x("delete from oauth_codes where key_id=?", a.key)
        db.x("delete from keys where key_id=?", a.key)
        db.x("delete from pending_keys where key_id=?", a.key)
        if not jobs: db.x("delete from credits where key_id=?", a.key)
    kept = "ledger and %d job row(s) kept for statements" % len(jobs) if jobs else "credit rows removed (key never ran a job)"
    print(f"revoked {a.key} (balance was ${bal:.2f}); {kept}")
    E.log("admin.key_revoke", key_id=a.key, balance_usd=bal, jobs=len(jobs), live=len(live))
...
def statement_text(db, cfg, k, month, start, end):
    ...
    L += ["", "Jobs (listed in the month they finished)", f"  {'finished':<21}{'id':<23}{'label':<24}{'state':<10}{'error':<14}{'gpu':<9}{'billed_s':>9}  cost"]
    ...
    for j in jobs:
        ...
        L.append(f"  {j['finished']:<21}{j['id']:<23}{label:<24}{j['state']:<10}{(j['error'] or '-')[:13]:<14}{(j['gpu'] or '-'):<9}{sec:>9d}  ${cost:.4f}")
...
def main(argv=None):
    ...
    rv = key.add_parser("revoke", help="delete a key so it can never authenticate again"); rv.add_argument("key", metavar="KEY_ID"); rv.add_argument("--force", action="store_true", help="revoke even with live jobs (their pods keep running)"); rv.set_defaults(f=cmd_key_revoke)
