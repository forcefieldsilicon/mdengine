...
(/v1/*), blob (/blob/*), and pod-facing (/internal/*) APIs, plus:
  ...
  * OAuth 2.1 for the MCP remote (mde_oauth.py): /.well-known/oauth-*, /oauth/register|authorize|token;
    the consent page takes the API key once, tokens (mat_...) act as that key.
...
class Config:
    def __init__(self, env=None):
        ...
        # Keyless / bad-key requests to /mcp per client IP per minute (handshake + tools/list are open; this caps abuse).
        self.keyless_per_min = int(e.get("MDE_MCP_KEYLESS_PER_MIN", "120") or 120)
...
create table if not exists oauth_clients(client_id text primary key, metadata text not null, created text not null);
create table if not exists oauth_codes(code_hash text primary key, client_id text not null, redirect_uri text not null, code_challenge text not null,
  key_id text not null, scope text, expires integer not null, used integer default 0);
create table if not exists oauth_tokens(token_hash text primary key, kind text not null, key_id text not null, client_id text not null, scope text,
  expires integer not null, created text not null, revoked integer default 0, parent_hash text);
...
class App:
    def __init__(self, cfg: Config):
        ...
        self.oauth = mde_oauth.OAuth(self)
        self._rl = {}; self._rl_lock = threading.Lock()      # ip -> [tokens, last_ts] token bucket for keyless /mcp

    def allow_keyless(self, ip):
        """Token bucket per IP: cfg.keyless_per_min tokens, refilled continuously. False -> 429."""
        cap = float(self.cfg.keyless_per_min); t = time.time()
        with self._rl_lock:
            tokens, last = self._rl.get(ip, [cap, t])
            tokens = min(cap, tokens + (t - last) * cap / 60.0)
            if tokens < 1: self._rl[ip] = [tokens, t]; return False
            self._rl[ip] = [tokens - 1, t]
            if len(self._rl) > 10000: self._rl = {k: v for k, v in self._rl.items() if t - v[1] < 120}
            return True

    def delete_job_files(self, j):
        """DELETE /v1/jobs/{id}/results and the MCP delete_results tool: remove deck + result tarballs now
        instead of at the 30-day purge. Only for finished jobs; metadata/billing rows stay."""
        if j["state"] not in TERMINAL: return 409, {"error": "not finished", "message": f"state is {j['state']}; cancel first"}
        gone = [n for n in (f"{j['id']}.in.tar.gz", f"{j['id']}.out.tar.gz") if self.unlink_blob(n, job=j["id"], reason="user_delete")]
        log("job.files_deleted", job=j["id"], key_id=j["key_id"], files=len(gone))
        return 200, {"id": j["id"], "deleted": gone, "note": "job metadata and billing records are kept; results can no longer be downloaded"}
    ...
    def job_results(self, j):
        ...
        if not sz and not os.path.exists(f):
            return 410, {"error": "results unavailable", "message": "no results file: deleted by request, or the run produced none"}
    ...
    def purge_blobs_once(self):
        ...
        try: self.oauth.purge_expired()
        except Exception as e: log("oauth.purge_error", error=repr(e))
...
input[type=password]{{background:#0b0d10;color:#e6e8eb;border:1px solid #232a33;border-radius:8px;padding:10px 12px;margin-top:6px}}button{{background:#7cb7ff;color:#0b0d10;border:0;border-radius:8px;padding:10px 18px;font-weight:600;cursor:pointer}}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def api_key(self):
        ...
        if b.startswith("mde_"): return self.app.db.key_by_hash(sha256(b))
        if b.startswith("mat_"): return self.app.oauth.key_for_access_token(b)     # OAuth access token -> the key that consented
        return None
    def redirect(self, url): self.send_response(302); self.send_header("Location", url); self.send_header("Cache-Control", "no-store"); self.send_header("Content-Length", "0"); self.end_headers()
    def client_ip(self): return (self.headers.get("X-Forwarded-For") or "").split(",")[0].strip() or self.client_address[0]
    ...
    # ------------------------------------------------------------------ GET
    def do_HEAD(self):
        """HEAD = GET without a body (uptime checkers, curl -I). Only for routes that need no auth."""
        p = urlparse(self.path).path.rstrip("/")
        if p == "/v1/health": self.send_response(200); self.send_header("Content-Type", "application/json"); self.end_headers(); return
    ...
    def do_GET(self):
        ...
        if p in ("/favicon.ico", "/favicon.png", "/icon.png"):        # directory listings + clients take the icon from the MCP host's favicon
            f = os.path.join(os.path.dirname(os.path.abspath(__file__)), "icon.png")
            if not os.path.exists(f): return self.send(404, {"error": "no icon"})
            with open(f, "rb") as fh: return self.send(200, raw=fh.read(), ctype="image/png", extra={"Cache-Control": "public, max-age=86400"})
        ...
        if p in ("/.well-known/oauth-protected-resource", "/.well-known/oauth-protected-resource/mcp"): return self.send(200, self.app.oauth.protected_resource_metadata(), extra=mde_mcp.CORS)
        if p == "/.well-known/oauth-authorization-server": return self.send(200, self.app.oauth.server_metadata(), extra=mde_mcp.CORS)
        if p == "/oauth/authorize": return self.oauth_authorize_get(parse_qs(u.query))
        ...
        if parts[1] != "v1": return self.send(404, {"error": "no route"})      # 404 not 401: a 401 on /.well-known/oauth-* makes MCP clients start an OAuth flow we do not offer
        k = self.api_key()
...
<div class=card><p class=muted>The full key is shown only once, right after payment. If you did not save it, <code>mdengine account</code> works if you already logged in; otherwise reply to your Stripe receipt and quote the key id above and we will issue a replacement.</p></div>""")
    ...
    def do_POST(self):
        ...
        if p.startswith("/oauth/"): return self.oauth_post(p)
        if parts[1] == "internal":                                   # pod side: heartbeat / done
            ...
            return self.send(404, {"error": "no route"})
        if parts[1] != "v1": return self.send(404, {"error": "no route"})      # e.g. POST /register from OAuth DCR probes
        k = self.api_key()
        if not k: return self.send(401, {"error": "bad api key"})
        if p == "/v1/jobs":
            spec = self.json_body()
    ...
    def mcp(self):
        ...
        if key is None:
            ip = self.client_ip()
            if not self.app.allow_keyless(ip):
                log("mcp.rate_limited", ip=ip)
                return self.send(429, {"error": "rate limited", "message": "too many unauthenticated requests; authenticate or slow down"}, extra={"Retry-After": "30", **mde_mcp.CORS})
        code, obj = self.app.mcp.handle(raw, key=key, bearer=self.bearer())
        extra = dict(mde_mcp.CORS)
        if code == 401: extra["WWW-Authenticate"] = self.app.oauth.www_authenticate(invalid=bool(self.bearer()))   # starts the OAuth flow in MCP clients
        return self.send(code, obj, extra=extra)

    # ------------------------------------------------------------------ OAuth 2.1 (mde_oauth.py)
    def oauth_error(self, e, extra=None):
        return self.send(e.status, {"error": e.error, "error_description": e.description}, extra={**mde_mcp.CORS, **(extra or {})})

    def oauth_authorize_get(self, q):
        oa = self.app.oauth
        if not self.app.allow_keyless(self.client_ip()): return self.html(429, "<h1>Slow down</h1><p class=muted>Too many requests from your network. Try again in a minute.</p>")
        try: meta, p = oa.check_authorize_request(q)
        except mde_oauth.OAuthError as e:
            return self.html(400, "<h1>Cannot start sign-in</h1><p class=warn>%s</p><p class=muted>The app that sent you here is misconfigured; nothing was authorized.</p>" % esc(e.description))
        err = oa.authorize_errors(p)
        if err: return self.redirect(oa.redirect_with(p["redirect_uri"], error=err[0], error_description=err[1], state=p["state"]))
        return self.html(200, oa.consent_page(meta, p))

    def oauth_post(self, p):
        oa = self.app.oauth; ip = self.client_ip()
        if not self.app.allow_keyless(ip):
            log("oauth.rate_limited", ip=ip, path=p)
            return self.send(429, {"error": "rate limited"}, extra={"Retry-After": "30", **mde_mcp.CORS})
        raw = self.body()
        if len(raw) > 65536: return self.send(413, {"error": "body too large"})
        ctype = (self.headers.get("Content-Type") or "").split(";")[0].strip().lower()
        if ctype == "application/json":
            try: form = json.loads(raw or b"{}")
            except json.JSONDecodeError: return self.send(400, {"error": "invalid_request", "error_description": "bad json"})
        else:
            form = {k: v[0] for k, v in parse_qs(raw.decode("utf-8", "replace"), keep_blank_values=True).items()}
        if p == "/oauth/register":
            try: return self.send(201, oa.register(form), extra=mde_mcp.CORS)
            except mde_oauth.OAuthError as e: return self.oauth_error(e)
        if p == "/oauth/token":
            try: return self.send(200, oa.token(form if isinstance(form, dict) else {}), extra={"Pragma": "no-cache", **mde_mcp.CORS})
            except mde_oauth.OAuthError as e: return self.oauth_error(e)
        if p == "/oauth/authorize":                              # consent form submit
            if not isinstance(form, dict): return self.html(400, "<h1>Bad request</h1>")
            try: meta, prm = oa.check_authorize_request(form)
            except mde_oauth.OAuthError as e: return self.html(400, "<h1>Cannot complete sign-in</h1><p class=warn>%s</p>" % esc(e.description))
            err = oa.authorize_errors(prm)
            if err: return self.redirect(oa.redirect_with(prm["redirect_uri"], error=err[0], error_description=err[1], state=prm["state"]))
            code = oa.issue_code(meta, prm, str(form.get("api_key") or "").strip())
            if not code:
                log("oauth.bad_key", client_id=meta["client_id"], ip=ip)
                return self.html(200, oa.consent_page(meta, prm, error="That API key was not recognised. Keys look like mde_ followed by 32 characters."))
            return self.redirect(oa.redirect_with(prm["redirect_uri"], code=code, state=prm["state"]))
        return self.send(404, {"error": "no route"})
    ...
    def do_DELETE(self):
        ...
        if parts[1] != "v1": return self.send(404, {"error": "no route"})
        k = self.api_key()
        if not k: return self.send(401, {"error": "bad api key"})
        if len(parts) not in (4, 5) or parts[1] != "v1" or parts[2] != "jobs": return self.send(404, {"error": "no route"})
        j = db.job(parts[3])
        if not j or j["key_id"] != k["key_id"]: return self.send(404, {"error": "no job"})
        if len(parts) == 5:
            if parts[4] != "results": return self.send(404, {"error": "no route"})
            return self.send(*self.app.delete_job_files(j))
